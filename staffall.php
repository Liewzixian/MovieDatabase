...
<audio src="Images/background.mp3" type="audio/mp3" autoplay loop>
      ...
      <center><h1>Staff Records</h1></center>
...
if ($query_run)
{
    if (mysqli_num_rows($query_run) > 0)
    {
        ...
        {
            echo "<tr><td>" . $row["staff_id"] . " </td><td> " . $row["first_name"] . "</td><td>" . $row["last_name"] . "</td><td> ". $row["address_id"] . "</td><td> ". $row["picture"] . "</td><td> ". $row["email"] . "</td><td> "
             . $row["store_id"] . "</td><td> ". $row["active"] . "</td><td> ". $row["username"] . "</td><td> ". $row["password"] . "</td><td> ". $row["last_update"] . "</td></tr>";
        }
    }
    else
    {
...
?>
<a href ="staff.php" class ="btn_back3">Back</a>
</table>
</div>
</div>
<br/>
